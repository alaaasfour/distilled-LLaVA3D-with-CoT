#!/usr/bin/env python3
"""Train distilled LLaVA-3D using pre-generated teacher responses."""
...
class DistillationTrainer:
    ...
    def train_epoch(self, dataloader: DataLoader, epoch: int):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            max_len = max(seq.size(0) for seq in question_tokens)
            question_tokens = torch.nn.utils.rnn.pad_sequence(
                question_tokens, batch_first=True, padding_value=0
            )
            answer_tokens = torch.nn.utils.rnn.pad_sequence(
                answer_tokens, batch_first=True, padding_value=0
            )
