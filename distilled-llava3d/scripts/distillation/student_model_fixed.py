#!/usr/bin/env python3
"""Student model architecture for distilled LLaVA-3D - Fixed version."""
...
class DistilledLLaVA3D(nn.Module):
    ...
    def analyze_image_content(self, pixel_values):
        ...
        with torch.no_grad():
            ...
            max_rgb = torch.max(raw_pixels, dim=(1, 2))[0]  # (3,) - Fixed: use [0] to get values
            min_rgb = torch.min(raw_pixels, dim=(1, 2))[0]  # (3,) - Fixed: use [0] to get values
